...
def save_file_modification(subject_id, total_blocks):
    ...
    variable_strings = ['condition', 'cue_present', 'type_of_trial', 'stimulus_one', 'stimulus_two', 'catch_trial', 'arrow_direction', 'correct_incorrect_late', 'iti']

    for ws_name in range(len(total_blocks)):
        data_wb.create_sheet(block_number_strings[ws_name]) # creating a sheet to save each block 
        
        data_sheet = data_wb[block_number_strings[ws_name]]
